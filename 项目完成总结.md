...
- ✅ **多分辨率支持**
  ...
  - 640x640x1024 (8x8mm扫描)
  - 980x980x1024 (12x12mm扫描)
  - 自动计算体素大小（可扩展到任意分辨率）
...
### 2. 文件输出
每次转换生成5个文件，保存在 `Results/<文件夹名>/` 目录：
- `OCTA_<文件夹>.tif` - Imaris兼容的3D TIFF（体素大小已嵌入）
- `OCTA_<文件夹>.nii.gz` - NIfTI格式（医学影像软件通用格式）
...
**医学影像软件（ITK-SNAP, 3D Slicer, FSL等）：**
1. 打开 `OCTA_<文件夹>.nii.gz`
2. 体素大小已嵌入NIfTI头文件
3. 可直接进行3D渲染和分析
