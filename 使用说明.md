...
### 2. NIfTI文件 (`OCTA_<文件夹>.nii.gz`)
- 医学影像通用格式（压缩）
- 3D图像数据（X, Y, Z）轴顺序
- 体素大小嵌入在NIfTI头文件
- **兼容ITK-SNAP, 3D Slicer, FSL, SPM等软件**
- 位置：`Results/<文件夹>/OCTA_<文件夹>.nii.gz`

### 3. NumPy数组 (`OCTA_<文件夹>.npy`)
- uint8格式
- 形状：(Y, X, Z)
- 用于Python后处理
- 位置：`Results/<文件夹>/OCTA_<文件夹>.npy`

### 4. 元数据 (`OCTA_<文件夹>_metadata.json`)
包含：
- 源文件信息
- 图像尺寸
- 体素大小（µm）
...
Results/
├── E361/
│   ├── OCTA_E361.tif                (Imaris格式)
│   ├── OCTA_E361.nii.gz             (NIfTI格式)
│   ├── OCTA_E361.npy                (NumPy数组)
│   ├── OCTA_E361_metadata.json      (元数据)
│   └── OCTA_E361_Preview.png        (预览图)
├── HenkE433/
│   ├── OCTA_HenkE433.tif
│   ├── OCTA_HenkE433.nii.gz
│   ├── OCTA_HenkE433.npy
│   ├── OCTA_HenkE433_metadata.json
│   └── OCTA_HenkE433_Preview.png
└── HenkE436/
    ├── OCTA_HenkE436.tif
    ├── OCTA_HenkE436.nii.gz
    ├── OCTA_HenkE436.npy
...
| 640x640 | 8x8 mm | ~12.5 µm |
| 730x730 | 9x9 mm | ~12.3 µm |
| 980x980 | 12x12 mm | ~12.2 µm |

**深度方向**：通常为2mm / 1024像素 ≈ 1.95 µm

⚠️ **注意**：脚本可以自动适应任意分辨率（基于~12µm/像素估算）。如果需要精确的扫描参数，请修改脚本中的`calculate_voxel_size`函数。
